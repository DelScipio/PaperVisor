    ...
    *,
            ...
            with ui.row().classes("w-full items-center gap-2 no-wrap"):
                ...
                if badge is not None:
                    if on_badge_menu is None:
                        ...
                        with ui.element("div").classes("pv-nav-badge-slot"):
                            with (
                                ui.button(badge, on_click=on_badge_menu)
                                .props("flat dense no-caps")
                                .classes("pv-chip pv-nav-chip-btn")
                            ):
                                pass
    ...
    with ui.element("div").classes("pv-nav-section-header"):
        if on_toggle_navigation is not None:
            with (
                ui.button(on_click=on_toggle_navigation)
                .props("flat no-caps dense")
                .classes("pv-nav-section-toggle")
            ):
                ui.label("Navigation").classes("pv-nav-section-label")
                ui.icon(
                    "chevron_right" if navigation_collapsed else "expand_more"
                ).classes("pv-text-dimmer text-base")
        else:
            ui.label("Navigation").classes("pv-nav-section-label")
    ...
    if not navigation_collapsed:
        with ui.column().classes("w-full px-2 gap-0"):
            ...
            nav_button(
                icon="description",
                label="All Files",
                on_click=on_open_show_all,
                active=(av == "all"),
            )
            nav_button(
                icon="schedule",
                label="Recently added",
                on_click=(on_open_recent or on_open_dashboard),
                active=(av == "recent"),
            )
            nav_button(
                icon="bookmark",
    ...
    with ui.element("div").classes("pv-nav-section-header"):
        if on_toggle_libraries is not None:
            with (
                ui.button(on_click=on_toggle_libraries)
                .props("flat no-caps dense")
                .classes("pv-nav-section-toggle")
            ):
                ui.label("Libraries").classes("pv-nav-section-label")
                ui.icon(
                    "chevron_right" if libraries_collapsed else "expand_more"
                ).classes("pv-text-dimmer text-base")
        else:
            ui.label("Libraries").classes("pv-nav-section-label")
        ui.button(icon="add", on_click=on_add_library).props(
            "dense flat round size=sm"
        ).classes("pv-nav-add-btn")
    with ui.column().classes("w-full px-2 gap-0"):
            ...
            for lib in libraries:
                ...
                if bool(getattr(lib, "is_shared_with_me", False)):
                    if on_remove_shared_library is not None:
                        menu_items.append(
                            ("Remove shared", lambda l=lib: on_remove_shared_library(l))
                        )
                    if (
                        str(getattr(lib, "shared_role", "") or "") == "editor"
                        and on_share_library is not None
                    ):
                        menu_items.insert(
                            0, ("Share", lambda l=lib: on_share_library(l))
                        )
                ...
                with ui.element("div").classes(row_class):
                    with (
                        ui.button(on_click=lib_click)
                        .props("flat no-caps dense")
                        .classes(
                            "pv-nav-btn w-full block justify-start"
                            + (
                                " pv-nav-btn--active"
                                if (av == "library" and str(lib.id) == alib)
                                else ""
                            )
                        )
                    ):
                        with ui.row().classes("w-full items-center gap-2 no-wrap"):
                            ...
                            ui.label(lib.name).classes("text-sm")
                            ui.space()
                            with ui.element("div").classes("pv-lib-count"):
                                ui.badge(str(lib.paper_count)).props(
                                    f'color="{badge_color}"'
                                ).classes("pv-chip")
                    ...
                    if menu_items:
                        with (
                            ...
                            .classes("pv-lib-menu pv-action-dots")
                        ):
                            with ui.menu():
    ...
    with ui.element("div").classes("pv-nav-section-header"):
        if on_toggle_markers is not None:
            with (
                ui.button(on_click=on_toggle_markers)
                .props("flat no-caps dense")
                .classes("pv-nav-section-toggle")
            ):
                ui.label("Markers").classes("pv-nav-section-label")
                ui.icon(
                    "chevron_right" if markers_collapsed else "expand_more"
                ).classes("pv-text-dimmer text-base")
        else:
            ui.label("Markers").classes("pv-nav-section-label")
        if on_add_marker is not None or on_add_auto_marker is not None:
            if on_add_marker is not None and on_add_auto_marker is not None:
                with (
                    ...
                    .classes("pv-nav-menu-btn")
                ):
                    with ui.menu():
                        ui.menu_item("New Marker", on_click=lambda: on_add_marker())
                        ui.menu_item(
                            "New Auto Marker", on_click=lambda: on_add_auto_marker()
                        )
            elif on_add_marker is not None:
                ui.button(icon="add", on_click=on_add_marker).props(
                    "dense flat round size=sm"
                ).classes("pv-nav-add-btn")
            elif on_add_auto_marker is not None:
                ui.button(icon="add", on_click=on_add_auto_marker).props(
                    "dense flat round size=sm"
                ).classes("pv-nav-add-btn")
    ...
    def _render_marker(sh: MarkerItem) -> None:
        ...
        with ui.element("div").classes(
            "pv-lib-row w-full" + ("" if has_marker_menu else " pv-no-menu")
        ):
            with (
                ui.button(on_click=_on_select_marker)
                .props("flat no-caps dense")
                .classes(
                    "pv-nav-btn w-full block justify-start"
                    + (
                        " pv-nav-btn--active"
                        if (av == "marker" and str(sh.id) == amarker)
                        else ""
                    )
                )
            ):
                with ui.row().classes("w-full items-center gap-2 no-wrap"):
                    ...
                    ui.label(str(sh.name)).classes("text-sm")
                    ui.space()
                    with ui.element("div").classes("pv-lib-count"):
                        ui.badge(badge).props('color="primary"').classes("pv-chip")
                ...
                with (
                    ...
                    with ui.menu():
                        if on_edit_marker is not None:
                            ...
                            ui.menu_item(
                                "Edit",
                                on_click=lambda _e, s=sh, cb=edit_cb: (
                                    cb(s) if cb is not None else None
                                ),
                            )
                        if on_delete_marker is not None:
                            ...
                            ui.menu_item(
                                "Delete",
                                on_click=lambda _e, s=sh, cb=delete_cb: (
                                    cb(s) if cb is not None else None
                                ),
                            )

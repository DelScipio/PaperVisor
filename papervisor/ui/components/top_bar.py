    ...
    with ui.row().classes('w-full max-w-none items-center gap-3 px-4 py-2 flex-wrap lg:flex-nowrap'):
        with ui.row().classes('items-center gap-2 flex-nowrap'):
            if on_toggle_left is not None:
                ui.button(icon='menu', on_click=on_toggle_left).props('dense flat round aria-label="Menu"').classes('pv-topbar-btn').tooltip('Menu')

            if on_toggle_filters is not None:
                ui.button(icon='tune', on_click=on_toggle_filters).props('dense flat round aria-label="Filters"').classes('pv-topbar-btn').tooltip('Filters')

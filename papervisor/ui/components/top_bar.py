    ...
    with ui.row().classes('w-full max-w-none items-center gap-3 px-4 py-2 flex-wrap lg:flex-nowrap'):
        with ui.row().classes('items-center gap-2 flex-nowrap'):
            if on_toggle_left is not None:
                ui.button(icon='menu', on_click=on_toggle_left).props('dense flat round aria-label="Toggle navigation menu"').classes('pv-topbar-btn').tooltip('Toggle menu')

            if on_toggle_filters is not None:
                ui.button(icon='tune', on_click=on_toggle_filters).props('dense flat round aria-label="Toggle filters"').classes('pv-topbar-btn').tooltip('Filters')
        ...
        with ui.row().classes('items-center w-full lg:flex-1 lg:justify-end order-last lg:order-none'):
            with ui.row().classes('items-center pv-search-pair flex-nowrap w-full lg:max-w-[680px]'):
                with ui.input(placeholder='Search...').props('dense outlined clearable').classes('pv-search-input flex-1 min-w-0') as search:
        ...
        with ui.row().classes('items-center gap-2 flex-nowrap'):
            ...
            ui.button(icon='cloud_upload', on_click=import_handler).props('dense flat round aria-label="Upload files"').classes('pv-topbar-btn').tooltip('Upload files')

            if is_admin:
                ui.button(icon='settings', on_click=lambda: ui.navigate.to('/admin')).props('dense flat round aria-label="Administration settings"').classes('pv-topbar-btn').tooltip('Administration')

            if on_open_profile is not None:
                ui.button(icon='person', on_click=on_open_profile).props('dense flat round aria-label="User profile"').classes('pv-topbar-btn').tooltip('Profile')

            if on_logout is not None:
                ui.button(icon='logout', on_click=on_logout).props('dense flat round aria-label="Log out"').classes('pv-topbar-btn').tooltip('Log out')

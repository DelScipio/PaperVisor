...
from papervisor.services.google_books import (
    fetch_googlebooks_metadata,
    search_googlebooks_isbn,
)
...
class MetadataActions:
        ...
        try:
            ...
            if old_file_name and new_file_name and old_file_name != new_file_name:
                ...
                if len(parts) == 2 and parts[1].isdigit():
                    self._notify(
                        f"Name existed, saved as {new_file_name}", color="info"
                    )
    ...
    def download_paper_file(self) -> None:
        try:
            ...
            try:
                ...
                self._notify("Failed to resolve file path", color="negative")
                return
    ...
    async def copy_paper_file_path(self) -> None:
        try:
            ...
            await ui.run_javascript(
                f"navigator.clipboard.writeText({json.dumps(str(p))})"
            )
    ...
    async def fetch_doi(self):
        ...
        async with self._action_context("Fetching Crossref metadata…"):
            meta = await asyncio.to_thread(
                fetch_doi_metadata, doi=str(doi).strip(), timeout_s=timeout_sec
            )
            ...
            if meta:
                ...
                self._notify(
                    "Metadata updated. Review inside Edit tab.", color="positive"
                )
            else:
            ...
            def _get():
                ...
                for opt in opts:
                    if self.state.fetch_cancel_requested:
                        raise asyncio.CancelledError("Fetch canceled by user")
                    if opt == "openlibrary":
                        m = fetch_openlibrary_metadata(
                            isbn=str(isbn).strip(), timeout_s=timeout_sec
                        )
                        if m:
                            return m
                    elif opt == "google":
                        m = fetch_googlebooks_metadata(
                            isbn=str(isbn).strip(), timeout_s=timeout_sec
                        )
                        if m:
                            return m
    ...
    async def detect_isbn(self):
        ...
        async with self._action_context("Analyzing file…"):
            ...
            if not t:
                self._notify(
                    "No ISBN found (could not search online without title).",
                    color="info",
                )
                return
            ...
            def _search():
                ...
                for opt in opts:
                    ...
                    if opt == "google":
                        res = search_googlebooks_isbn(
                            title=t, author=a, timeout_s=timeout_sec
                        )
                        if res:
                            return res
                return None
    ...
    async def regen_snapshot(self):
        ...
        try:
            if str(p).lower().endswith(".epub"):
                ...
                self._notify(
                    "Preview only supported for PDF and EPUB.", color="warning"
                )
                return
    ...
    async def process_replace_upload(self, e):
        ...
        name = str(
            getattr(e, "name", None) or getattr(e, "filename", None) or "upload.bin"
        )
        ...
        try:
            if self.dialog._user_id is not None and not bool(
                self.state.can_manage_paper_library
            ):
        ...
        try:
            lib_id = str(
                self.paper.library_id
                or getattr(self.dialog.library_in, "value", None)
                or ""
            ).strip()

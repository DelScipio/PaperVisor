...
def kv_link_cell(*, label: str, value: str, href: str) -> tuple[ui.element, ui.link]:
    with ui.column().classes('min-w-0 gap-0 py-1') as c:
        ui.label(label).classes('pv-meta-kv-label')
        v = ui.link(value or '—', href, new_tab=True).classes('pv-meta-kv-value pv-meta-kv-link')
    return c, v
...
def render_details_view(
    ...
    with header_card:
        with ui.row().classes('w-full items-start justify-between gap-4'):
            with ui.column().classes('min-w-0 flex-1 gap-1'):
                ui.label(paper.title or '').classes('text-xl font-bold break-words tracking-tight')
            with ui.element('span').classes('pv-meta-type-badge shrink-0'):
                ui.label('Book' if (paper.file_type or 'paper') == 'book' else 'Paper')

        # File actions row
        with ui.row().classes('pv-meta-file-row w-full items-center justify-between gap-2 mt-3'):
    ...
    with cards_grid:
        ...
        with id_card:
            id_grid = ui.element('div').classes('w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-1')
            with id_grid:
                kv_cell(label='Title', value=paper.title or '')
                kv_cell(label='Authors', value=format_authors_et_al(paper.authors or ''))
                ...
                rows['pubdate'], _ = kv_cell(label='Publication', value=paper.publication_date or '')
                rows['lang'], _ = kv_cell(label='Language', value=paper.language or '')
                rows['series'], _ = kv_cell(label='Series', value=paper.series or '')
                rows['series_idx'], _ = kv_cell(label='Series #', value=paper.series_index or '')
                rows['pages'], _ = kv_cell(label='Pages', value=str(paper.page_count or ''))
                ...
                rows['keywords'], _ = kv_cell(label='Keywords', value=paper.keywords or '')
    ...
    with desc_card:
        ...
        with ui.column().classes('w-full gap-1'):
            desc_text = str(paper.description if (paper.file_type or 'paper') == 'book' else paper.abstract or '').strip()
            ...
            def _toggle_desc(e, state=state, c=view_desc_collapsed, f=view_desc_full, t=desc_toggle):

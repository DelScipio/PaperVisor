    ...
    *,
    ...
    if on_owner_user_id_change is not None:
        with ui.card().props("flat bordered").classes("pv-dialog-card w-full"):
            ...
            ui.label(
                "Select a user to limit operations to their libraries only"
            ...
            with ui.row().classes("w-full items-center gap-3 pt-2"):
                ...
                sel = (
                    ui.select(
                        user_opts, value=initial_value, label="User", with_input=True
                    )
    ...
    def _choose_libraries_with_options(
        *,
        ...
        with dlg, dialog_card(max_width_class="max-w-2xl"):
            ...
            # Option checkboxes
            if show_overwrite:
                option_widgets["overwrite"] = ui.checkbox(
                    "Overwrite existing data", value=False
                ).classes("text-sm")

            if show_dry_run:
                option_widgets["dry_run"] = ui.checkbox(
                    "Dry run (preview only, no changes)", value=True
                ).classes("text-sm")
                with ui.row().classes("w-full items-center gap-2"):
                    ui.icon("info", color="blue").classes("text-sm")
                    ui.label("Preview what will be changed before committing").classes(
                        "text-xs pv-text-dimmer"
                    )

            if show_fetch_covers:
                option_widgets["fetch_covers"] = ui.checkbox(
                    "Also fetch cover images", value=True
                ).classes("text-sm")

            if extra_options:
                for opt_label, opt_default in extra_options:
                    key = opt_label.lower().replace(" ", "_")
                    option_widgets[key] = ui.checkbox(
                        opt_label, value=opt_default
                    ).classes("text-sm")
            ...
            async def _confirm(_e=None) -> None:
                try:
                    ...
                    if bool(all_toggle.value):
                        ...
                        selected = [
                            lid for (lid, c) in lib_checks.items() if bool(c.value)
                        ]
                        if not selected:
            ...
            with dialog_actions_row():
                ui.button("Cancel", on_click=dlg.close).props(
                    "flat color=negative"
                ).classes("pv-meta-action-btn")
                ui.button("Run", on_click=_confirm).props("color=primary").classes(
                    "pv-meta-save-btn"
                )
    ...
    async def _run_with_progress(
        *,
        ...
        try:
            ...
            logger.error("Error during maintenance job", exc_info=ex)
            ui.notify(
                "An unexpected error occurred during maintenance operations.",
                color="negative",
            )
            return None
        finally:
        ...
        try:
            while True:
                ...
                if total > 0:
                    ...
                    progress_label.text = (
                        f"{label}: {current}/{total}{eta_part} — {truncated}"
                    )
                elif status == "queued":
                    if queued_total > 0:
                        progress_label.text = (
                            f"{label}: queued ({queued_ahead + 1}/{queued_total})…"
                        )
                    else:
                ...
                if status == "failed":
                    logger.error(
                        "Metadata job failed: %s", snap.error or "Unknown error"
                    )
                    ui.notify(
                        "An unexpected error occurred during maintenance operations.",
                        color="negative",
    ...
    with ui.card().props("flat bordered").classes("pv-dialog-card w-full mt-2"):
        ...
        ui.label("Sync, clean, and optimize your libraries").classes(
            "text-xs pv-text-dimmer"
        )

        with ui.expansion("Clean Libraries", icon="cleaning_services").classes(
            "w-full mt-2"
        ):
            ui.markdown("""
            ...
            **Safety:** Use dry-run first to preview what will change
            ...
            def _clean_click() -> None:
                async def _go(library_ids: list[str] | None, opts: dict) -> None:
                    ...
                    r = await _run_with_progress(
                        ...
                        runner=lambda: clean_libraries(
                            library_ids=library_ids, dry_run=dry_run, on_progress=cb
                        ),
                    ...
                    if r is not None:
                        ...
                        ui.notify(
                            msg, color="info" if dry_run else "positive", timeout=8000
                        )
            ...
            with ui.row().classes("w-full justify-end pt-2"):
                ui.button(
                    "Run Clean", on_click=_clean_click, icon="cleaning_services"
                ).props("color=primary").classes("pv-meta-save-btn")
        ...
        with ui.expansion("Regenerate PDF Thumbnails", icon="image").classes("w-full"):
            ...
            **Note:** Only processes PDF files. EPUB covers use embedded images.
            ...
            with ui.row().classes("w-full justify-end pt-2"):
                ui.button("Run", on_click=_regen_thumbs_click, icon="refresh").props(
                    "outline"
                ).classes("pv-meta-action-btn")
    ...
    with ui.card().props("flat bordered").classes("pv-dialog-card w-full mt-2"):
        ...
        ui.label("Fetch and enrich metadata from external sources").classes(
            "text-xs pv-text-dimmer"
        )

        with ui.expansion("Extract EPUB Covers", icon="auto_stories").classes(
            "w-full mt-2"
        ):
            ui.markdown("""
            ...
            **Process:** Extracts cover from EPUB metadata or first image in book
            ...
            def _epub_covers_click() -> None:
                async def _go(library_ids: list[str] | None, opts: dict) -> None:
                    ...
                    r = await _run_with_progress(
                        ...
                        runner=lambda: extract_epub_covers(
                            library_ids=library_ids, overwrite=overwrite, on_progress=cb
                        ),
                        progress_bar=progress_bar,
                        progress_label=progress_label,
                    )
            ...
            with ui.row().classes("w-full justify-end pt-2"):
                ui.button("Run", on_click=_epub_covers_click, icon="collections").props(
                    "outline"
                ).classes("pv-meta-action-btn")

        with ui.expansion("Fetch DOI Metadata (Papers)", icon="article").classes(
            "w-full"
        ):
            ui.markdown("""
            ...
            **Note:** DOI is auto-extracted from PDF files if not already set
            ...
            with ui.row().classes("w-full justify-end pt-2"):
                ui.button("Run", on_click=_doi_click, icon="science").props(
                    "outline"
                ).classes("pv-meta-action-btn")

        with ui.expansion(
            "Fetch ISBN Metadata & Covers (Books)", icon="library_books"
        ).classes("w-full"):
            ui.markdown("""
            ...
            **Note:** ISBN is auto-detected from EPUB/PDF contents or filename if not already set
            ...
            with ui.row().classes("w-full justify-end pt-2"):
                ui.button("Run", on_click=_isbn_click, icon="barcode_scanner").props(
                    "outline"
                ).classes("pv-meta-action-btn")
    ...
    with ui.card().props("flat bordered").classes("pv-dialog-card w-full mt-2"):
        with ui.expansion("Advanced Operations", icon="warning").classes("w-full"):
            ui.label("These operations are rarely needed. Use with caution.").classes(
                "text-xs text-orange pt-1"
            )
            ...
            with ui.card().props("flat bordered").classes("pv-dialog-card w-full"):
                ui.label("Fetch Book Covers (standalone)").classes(
                    "text-sm font-semibold pv-text-dim"
                )
                ui.label(
                    'Download covers for books that already have ISBNs set. For most use cases, use "Fetch ISBN Metadata & Covers" instead.'
                ).classes("text-xs pv-text-dimmer")
                ...
                def _covers_click() -> None:
                    async def _go(library_ids: list[str] | None, opts: dict) -> None:
                        ...
                        r = await _run_with_progress(
                            ...
                            runner=lambda: fetch_book_covers(
                                library_ids=library_ids, on_progress=cb
                            ),
                            progress_bar=progress_bar,
                            progress_label=progress_label,
                        )
                ...
                with ui.row().classes("w-full justify-end pt-2"):
                    ui.button("Run", on_click=_covers_click, icon="download").props(
                        "outline"
                    ).classes("pv-meta-action-btn")
            ...
            # --- User Data Cleanup ---
            with ui.card().props("flat bordered").classes("pv-dialog-card w-full"):
                ui.label("User Data Cleanup").classes(
                    "text-sm font-semibold pv-text-dim"
                )
                ui.label(
                    "Remove orphaned database records and files from deleted users"
                ).classes("text-xs pv-text-dimmer")

                with ui.expansion("What does this do?", icon="help_outline").classes(
                    "w-full mt-2"
                ):
                    ui.markdown("""
                ...
                dry_run_cb = ui.checkbox(
                    "Dry run (preview only, no actual deletions)", value=True
                ).classes("text-sm pt-2")
                delete_dirs_cb = ui.checkbox(
                    "Also delete orphaned user folders from disk", value=False
                ).classes("text-sm")

                with ui.row().classes("w-full items-center gap-2 pt-2"):
                    ui.icon("warning", color="orange").classes("text-base")
                    ui.label(
                        "Recommended: Run with dry run enabled first to review what will be deleted"
                ...
                async def _run_user_cleanup(_e=None) -> None:
                    ...
                    try:
                        ...
                        ui.notify(
                            msg,
                            color="positive" if not r.dry_run else "info",
                            timeout=8000,
                        )
                        on_changed()
                    except Exception as ex:
                        logger.error("Error during user cleanup", exc_info=ex)
                        ui.notify(
                            "An unexpected error occurred during maintenance operations.",
                            color="negative",
                        )

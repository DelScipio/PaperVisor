...
def index() -> None:
    ...
    def _restore_last_opened_location() -> None:
        remember_mode = resolve_remember_location_mode(
            user_id=user_id, logger_context="index page"
        )
        ...
        last_view = str(
            get_user_setting(user_id=user_id, key="nav.last.view", default="") or ""
        ).strip()
        last_library_id = (
            str(
                get_user_setting(user_id=user_id, key="nav.last.library_id", default="")
                or ""
            ).strip()
            or None
        )
        last_marker_id = (
            str(
                get_user_setting(user_id=user_id, key="nav.last.marker_id", default="")
                or ""
            ).strip()
            or None
        )
    ...
    if user_id and not has_handoff_intent:
        try:
            ...
            logger.debug(
                "Failed restoring last opened location for user_id=%s",
                user_id,
                exc_info=True,
            )

    # Initialize display_mode from user settings
    state.display_mode = str(
        get_user_setting(user_id=user_id, key="ui.display_mode", default="grid")
        or "grid"
    ).strip()
...
(function(){{
    ...
    def open_reader(paper: PaperItem) -> None:
        def _open_in_viewer(
            *, viewer_path: str, file_path: str, fallback_title: str
        ) -> None:
        ...
        try:
            ...
            if (
                ...
                and str(getattr(paper_row, "title", "") or "").strip()
            ):
                title = str(paper_row.title).strip()
        except Exception:
            logger.debug(
                "Failed resolving reader title for paper_id=%s", paper.id, exc_info=True
            )
        ...
        try:
            ...
            logger.debug(
                "Failed resolving reader file extension for paper_id=%s",
                paper.id,
                exc_info=True,
            )
            ext = ""
        ...
        if ext == ".epub":
            _open_in_viewer(
                viewer_path="/static/epub_viewer.html",
                file_path="raw",
                fallback_title="Book",
            )
            return
        ...
        if ext == ".cbz":
            _open_in_viewer(
                viewer_path="/static/cbz_viewer.html",
                file_path="raw",
                fallback_title="Comic",
            )
            return

        _open_in_viewer(
            viewer_path="/static/pdfjs/web/viewer.html",
            file_path="file",
    ...
    def render_left_nav() -> None:
        ...
        _toggle_navigation, _toggle_libraries, _toggle_markers, _toggle_auto_markers = (
            make_nav_toggle_handlers_attr(
                state=state,
                user_id=user_id,
    ...
    def render_filters() -> None:
        ...
        try:
            ...
            logger.debug(
                "Failed loading markers for filters panel (user_id=%s)",
                user_id,
                exc_info=True,
            )
            markers = []
        ...
        try:
            facets = list_paper_filter_facets(
                user_id=user_id, library_ids=eff_library_ids, file_type=ft_val, limit=50
            )
        except Exception:
            logger.debug(
                "Failed loading filter facets for user_id=%s view=%s",
                user_id,
                view,
                exc_info=True,
            )
        ...
        if filters_header_menu is not None:
            try:
                ...
                logger.debug(
                    "Failed clearing filters header menu container", exc_info=True
                )
    ...
    def _close_right_drawer(drawer) -> None:
        close_filters_drawer_and_persist(
            drawer=drawer, user_id=user_id, context="index"
        )
    ...
    def render_right_drawer() -> None:
        ...
        with (
            ui.right_drawer(value=drawer_open)
            .props("width=320 bordered breakpoint=900")
            .classes("pv-surface") as right_drawer
        ):
            with ui.row().classes("pv-drawer-header pv-filters-drawer-header w-full"):
                ...
                ) as filters_header_menu:
                    pass
                ui.button(
                    icon="close",
                    on_click=lambda _e, d=right_drawer: _close_right_drawer(d),
                ).props("flat dense round").classes("pv-drawer-close")
            render_filters()

    def _set_view(
        view: str, library_id: str | None, marker_id: str | None = None
    ) -> None:
    ...
    def toggle_right() -> None:
        if right_drawer:
            toggle_filters_drawer_and_persist(
                drawer=right_drawer, user_id=user_id, default_open=False
            )
    ...
    with page_scaffold(
        ...
        # ... (Same as before)
        kb_info_btn = (
            ui.button("", on_click=lambda _e=None: None)
            .props("id=pv_kb_info")
            .classes("hidden")
        )
        kb_reader_btn = (
            ui.button("", on_click=lambda _e=None: None)
            .props("id=pv_kb_reader")
            .classes("hidden")
        )
        kb_fav_btn = (
            ui.button("", on_click=lambda _e=None: None)
        ...
        async def _kb_action_on_selected(action: str) -> None:
            ...
            paper_item = PaperItem(
                ...
                open_count_since_reset=int(
                    getattr(paper_row, "open_count_since_reset", 0) or 0
                ),
        ...
        def render_content() -> None:
            ...
            def _render_library_breadcrumb() -> None:
                ...
                with ui.button("Libraries").props("flat dense no-caps") as _lib_btn:
                    ...
                    with ui.menu().classes("min-w-[170px]"):
                        if not libs:
                            ui.menu_item("No libraries yet").props(
                                "disable dense"
                            ).classes("text-xs py-1")
                        else:
                            for lib in libs:
                                ...
                                def make_library_handler(lib_id: object):
                                    def handler() -> None:
                                        _breadcrumb_nav(
                                            "library", library_id=lib_id, marker_id=None
                                        )
                                ...
                                item = (
                                    ui.menu_item(
                                        lname, on_click=make_library_handler(lid)
                                    )
                                    .props("dense")
                                    .classes("text-xs py-1")
                                )
                ...
                _breadcrumb_sep()
                ui.label(lib_name_by_id.get(library_id, "Library")).classes(
                    "pv-bc-current"
                )
            ...
            def _render_marker_breadcrumb() -> None:
                ...
                with ui.button("Markers").props("flat dense no-caps") as _marker_btn:
                    ...
                    with ui.menu().classes("min-w-[170px]"):
                        if not markers:
                            ui.menu_item("No markers yet").props(
                                "disable dense"
                            ).classes("text-xs py-1")
                        else:
                            for m in markers:
                                ...
                                item = (
                                    ui.menu_item(
                                        mname, on_click=make_marker_handler(mid)
                                    )
                                    .props("dense")
                                    .classes("text-xs py-1")
                                )
                ...
                _breadcrumb_sep()
                ui.label(marker_name_by_id.get(marker_id, "Marker")).classes(
                    "pv-bc-current"
                )
                ...
                with ui.row().classes("w-full items-center justify-between mb-2"):
                    with ui.element("div").classes("pv-breadcrumbs"):
                        ui.link("Dashboard", "#").classes("pv-bc-link").on(
                            "click.prevent",
                            lambda _e=None: _breadcrumb_nav("dashboard"),
                        )
                    ...
                    def _set_display_mode(mode: str) -> None:
                        ...
                        set_user_setting(
                            user_id=user_id, key="ui.display_mode", value=mode
                        )
                        if "render_content" in refreshables:
                            refreshables["render_content"].refresh()
                    ...
                    with ui.row().classes("items-center gap-1 shrink-0"):
                        ui.button(
                            icon="grid_view",
                            on_click=lambda _e=None: _set_display_mode("grid"),
                        ).props(
                            f"flat dense round {'color=primary' if state.display_mode == 'grid' else 'color=grey-5'}"
                        ).tooltip("Grid view")
                        ui.button(
                            icon="view_list",
                            on_click=lambda _e=None: _set_display_mode("list"),
                        ).props(

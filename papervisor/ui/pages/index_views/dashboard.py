...
def _render_dashboard_stat_card(
    *, label: str, value: int, icon: str, color: str
) -> None:
...
def _render_most_opened_section(
    *,
    ...
    def _render_corner_toggle_button(
        *, icon: str, color: str, is_active: bool, tooltip: str, on_click
    ) -> None:
        ui.button(icon=icon, on_click=on_click).props(
            f'flat dense round color={color} aria-label="{tooltip}"'
        ).classes(
    ...
    def _render_most_opened_poster(*, paper: PaperItem, tooltip: str) -> None:
        def _render_poster_overlay_actions() -> None:
            with ui.element("div").classes("pv-poster-overlay-actions"):
                ui.button(
                    icon="info", on_click=lambda _e, pp=paper: open_metadata(pp)
                ).props('flat dense round aria-label="Metadata"').classes(
                    "pv-fab pv-fab--metadata"
                ).tooltip("Metadata")
                ui.button(
                    icon="description", on_click=lambda _e, pp=paper: open_reader(pp)
                ).props('flat dense round aria-label="Open reader"').classes(
                    "pv-fab pv-fab--read"
                ).tooltip("Open reader")
            ...
            with ui.element("div").classes("pv-poster-corner"):
                ...
                toggle_specs = [
                    ...
                    (
                        ...
                        lambda pid=paper.id: toggle_favorite(
                            paper_id=pid, user_id=user_id
                        ),
                    ),
                    (
                        "bookmark",
                        ...
                        lambda pid=paper.id: toggle_to_read(
                            paper_id=pid, user_id=user_id
                        ),
                    ),
                ]

                for icon, color, is_active, tooltip, action in toggle_specs:
                ...
                ui.button(
                    ...
                    'flat dense round color=primary aria-label="Reset open counts"'
                ).classes("pv-corner-btn pv-hover-only").tooltip("Reset open counts")

            with ui.element("div").classes("pv-poster-thumb"):
                prev_url = preview_image_url_for(paper_id=paper.id)
                if prev_url:
                    ui.image(prev_url).props("loading=lazy").classes(
    ...
    def _render_most_opened_header(on_scroll_left, on_scroll_right) -> None:
        with ui.row().classes(
            "w-full items-center justify-between px-4 pt-3 pv-dashboard-section-head"
        ):
            with ui.row().classes("items-center gap-3"):
                ui.label("Most opened").classes("text-base font-semibold")
                ui.label("Counts since reset and total opens.").classes(
                    "text-xs pv-text-dimmer"
                )
            with ui.row().classes("items-center gap-1"):
                ui.button(icon="chevron_left", on_click=on_scroll_left).props(
                    'flat dense round aria-label="Scroll left"'
                ).tooltip("Scroll left")
                ui.button(icon="chevron_right", on_click=on_scroll_right).props(
                    'flat dense round aria-label="Scroll right"'
    ...
    with ui.element("section").classes("pv-dashboard-section"):
        most_opened_row_dom_id, on_scroll_left, on_scroll_right = (
            _setup_most_opened_scroller()
        )
        _render_most_opened_header(
            on_scroll_left=on_scroll_left, on_scroll_right=on_scroll_right
        )

        with (
            ui.row()
            .props(f"id={most_opened_row_dom_id}")
            .classes("w-full gap-4 px-4 py-4 flex-nowrap pv-marker-row")
        ):
            if not papers:
